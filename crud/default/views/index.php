...
$this->title =
<?= ($generator->pluralize) ? $generator->generateString(Inflector::pluralize(Inflector::camel2words($baseModelClass))) : $generator->generateString(Inflector::camel2words($baseModelClass)) ?>;
...
<div class="<?= Inflector::camel2id($baseModelClass) ?>-index">
    ...
    <div class="flex justify-content-between align-items-center mb-3">
        ...
        <div class="btn-group">
            <?= "<?= " ?>Html::a(<?= $generator->generateString('Create ' . Inflector::camel2words($baseModelClass)) ?>,
            ['create'], ['class' => 'btn btn-success']) ?>
            <?php if (!empty($generator->searchModelClass) && $generator->advancedSearch): ?>
                <?= "<?= " ?>Html::a(<?= $generator->generateString('Advance Search') ?>, '#', ['class' => 'btn btn-info
                search-button']) ?>
            <?php endif; ?>

            <?php if (!empty($generator->searchModelClass) && $generator->advancedSearch): ?>
                <?= "    <?php " . ($generator->indexWidgetType === 'grid' ? "// " : "") ?>echo $this->render('_search',
                ['model' => $searchModel]); ?>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="shadow-sm p-3 bg-body rounded mt-3">
        ...
        if ($generator->indexWidgetType === 'grid'):
            ...
            'kv-pjax-container-<?= Inflector::camel2id(StringHelper::basename($generator->modelClass)) ?>']],
            'striped' => true,
            'bordered' => false,
            'panel' => [
            // 'heading' => false,
        ...
        <?php
            ...
            return $this->render('_index',['model' => $model, 'key' => $key, 'index' => $index, 'widget' => $widget, 'view'
            => $this]);
            },
            ]) ?>
        <?php
